...
class SortedList:
    ...
    def insert(self, value):
        ...
        current = self._front
        if current is None:
            self._front = _SLNode(value, None)
        elif value < current._data:
            self._front = _SLNode(value, current)
        else:            
            ...
            if current is not None:
                ...
                if current is None:
                    ...
                    if new_current is None:
                        new_list._front = _SLNode(temp._data,None)
                    else:
                        while new_current._next is not None:
                            new_current = new_current._next
                        new_current._next = _SLNode(temp._data, None)
                    new_list._count += 1
        ...
        if self._front is None:
            ...
            while temp is not None:
                ...
                if i == -1:
                    ...
                    if current is None:
                        self._front = _SLNode(temp._data, None)
                    elif temp._data < current._data:
                        self._front = _SLNode(temp._data, current)
                    else:            
                        while current is not None and temp._data >= current._data:
                            previous = current
                            current = current._next
                   
                        previous._next = _SLNode(temp._data,current)
                    self._count += 1
                temp = temp._next
